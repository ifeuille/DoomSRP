...
namespace DoomSRP
{
    public static class IFPipelineEditorTools
    {
        ...
        static public bool MakeTextureReadable(string path, bool force)
        {
            ...
            if (force || !settings.readable || settings.npotScale != TextureImporterNPOTScale.None || settings.alphaIsTransparency)
            {
                ...
                if (IFPipelineSettings.trueColorAtlas) settings.textureFormat = TextureImporterFormat.AutomaticTruecolor;
        ...
        static bool MakeTextureAnAtlas(string path, bool force, bool alphaTransparency)
        {
                ...
                settings.npotScale != TextureImporterNPOTScale.ToNearest)
            {
                ...
                if (IFPipelineSettings.trueColorAtlas)
                {
        ...
        static public string GetSaveableTexturePath(SpritesAtlas atlas)
        {
            ...
            if (atlas.texture != null)
            {
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    ...
                    return path.Substring(0, dot) + ".tga";
                }
            }

            // No texture to use -- figure out a name using the atlas
            path = AssetDatabase.GetAssetPath(atlas.GetInstanceID());
            path = string.IsNullOrEmpty(path) ? "Assets/" + atlas.name + "_t.tga" : path.Replace(".prefab", "_t.tga"); //atlas 重命名
            return path;
        }
        ...
        static public Rect IntRect(string prefix, Rect rect)
        {
            ...
            IFPipelineEditorTools.IntVector a = IFPipelineEditorTools.IntPair(prefix, "Left", "Top", left, top);
            IFPipelineEditorTools.IntVector b = IFPipelineEditorTools.IntPair(null, "Width", "Height", width, height);
        ...
        static public Vector4 IntPadding(string prefix, Vector4 v)
        {
            ...
            IFPipelineEditorTools.IntVector a = IFPipelineEditorTools.IntPair(prefix, "Left", "Top", left, top);
            IFPipelineEditorTools.IntVector b = IFPipelineEditorTools.IntPair(null, "Right", "Bottom", right, bottom);
        ...
        static public void DrawSprite(Texture2D tex, Rect rect, SpriteData sprite, Color color)
        {
        ...
        static public void DrawSprite(Texture2D tex, Rect drawRect, SpriteData sprite, Color color, Material mat)
        {
        ...
        {
            IFPipelineEditorTools.DrawSprite(tex, drawRect, color, null,
            ...
            int x, int y, int width, int height, int borderLeft, int borderBottom, int borderRight, int borderTop)
        {
            ...
            // Draw the background
            IFPipelineEditorTools.DrawTiledTexture(outerRect, IFPipelineEditorTools.backdropTexture);
            ...
            if (Selection.activeGameObject == null || Selection.gameObjects.Length == 1)
            {
                ...
                {
                    tex = IFPipelineEditorTools.contrastTexture;
                    GUI.color = Color.white;

                    if (borderLeft > 0)
                    {
                        float x0 = (float)borderLeft / width * outerRect.width - 1;
                        IFPipelineEditorTools.DrawTiledTexture(new Rect(x0, 0f, 1f, outerRect.height), tex);
                    }

                    if (borderRight > 0)
                    {
                        float x1 = (float)(width - borderRight) / width * outerRect.width - 1;
                        IFPipelineEditorTools.DrawTiledTexture(new Rect(x1, 0f, 1f, outerRect.height), tex);
                    }

                    if (borderBottom > 0)
                    {
                        float y0 = (float)(height - borderBottom) / height * outerRect.height - 1;
                        IFPipelineEditorTools.DrawTiledTexture(new Rect(0f, y0, outerRect.width, 1f), tex);
                    }

                    if (borderTop > 0)
                    {
                        float y1 = (float)borderTop / height * outerRect.height - 1;
                        IFPipelineEditorTools.DrawTiledTexture(new Rect(0f, y1, outerRect.width, 1f), tex);
                    }
                }
                GUI.EndGroup();
        ...
        static public void DrawSpriteField(string label, SpritesAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
        {
            GUILayout.BeginHorizontal();
        ...
        static public void DrawPaddedSpriteField(string label, SpritesAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(label, GUILayout.Width(76f));
            ...
            IFPipelineEditorTools.DrawPadding();
            GUILayout.EndHorizontal();
        }
        ...
        static public void DrawSpriteField(string label, string caption, SpritesAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(label, GUILayout.Width(76f));
        ...
        static public bool DrawSpriteField(SpritesAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
        {
            if (atlas.GetSprite(spriteName) == null)
                spriteName = "";

            if (IFPipelineEditorTools.DrawPrefixButton(spriteName, options))
            {
                IFPipelineSettings.atlas = atlas;
                IFPipelineSettings.selectedSprite = spriteName;
                SpriteSelector.Show(callback);
        ...
        static public void DrawSpriteField(string label, SerializedObject ob, SerializedProperty atlas, SerializedProperty sprite, float width, bool padded, bool removable, params GUILayoutOption[] options)
        {
            if (atlas != null && atlas.objectReferenceValue != null)
            {
                ...
                else
                {
                    ...
                    {
                        if (GUILayout.Button(spriteName, "MiniPullDown", options))
                            SpriteSelector.Show(ob, sprite, atlas.objectReferenceValue as SpritesAtlas);
                    }
        ...
        static public void DrawAdvancedSpriteField(SpritesAtlas atlas, string spriteName, SpriteSelector.Callback callback, bool editable,
            params GUILayoutOption[] options)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                if (IFPipelineEditorTools.DrawPrefixButton("Sprite"))
                {
                    IFPipelineSettings.atlas = atlas;
                    IFPipelineSettings.selectedSprite = spriteName;
                    SpriteSelector.Show(callback);
                ...
                //        {
                //            SpriteData sprite = atlas.GetSprite(spriteName);

                //            if (sprite != null)
                //            {
                //                IFPipelineEditorTools.RegisterUndo("Edit Sprite Name", atlas);
                //                sprite.name = newName;
                ...
                //                    {
                //                        IFPipelineEditorTools.RegisterUndo("Edit Sprite Name", sp);
        ...
        {
            //if (SpritesAtlasInspector.instance != null)
            //    SpritesAtlasInspector.instance.Repaint();

            //if (SpritesAtlasMaker.instance != null)
            //    SpritesAtlasMaker.instance.Repaint();
        ...
        static public void SelectSprite(string spriteName)
        {
            ...
            {
                IFPipelineSettings.selectedSprite = spriteName;
                IFPipelineEditorTools.Select(IFPipelineSettings.atlas.gameObject);
                RepaintSprites();
            }
        }
        ...
        static public void SelectSprite(SpritesAtlas atlas, string spriteName)
        {
            if (atlas != null)
            {
                IFPipelineSettings.atlas = atlas;
                IFPipelineSettings.selectedSprite = spriteName;
                IFPipelineEditorTools.Select(atlas.gameObject);
                RepaintSprites();
            }
        }
        ...
        static public bool DrawHeader(string text) { return DrawHeader(text, text, false, IFPipelineSettings.minimalisticLook); }

        /// <summary>
        /// Draw a distinctly different looking header label
        /// </summary>

        static public bool DrawHeader(string text, string key) { return DrawHeader(text, key, false, IFPipelineSettings.minimalisticLook); }
        ...
        static public void BeginContents() { BeginContents(IFPipelineSettings.minimalisticLook); }
        ...
        static public SerializedProperty DrawProperty(string label, SerializedObject serializedObject, string property, bool padding, params GUILayoutOption[] options)
        {
            ...
            if (sp != null)
            {
                if (IFPipelineSettings.minimalisticLook) padding = false;
        ...
        static public float SceneViewDistanceToRectangle(Vector3[] worldPoints, Vector2 mousePos)
        {
            ...
            return Math.DistanceToRectangle(screenPoints, mousePos);
        }
        ...
        static public Color32[] AddBorder(Color32[] colors, int width, int height)
        {
            ...
            {
                int y1 = Math.ClampIndex(y2 - 1, height);

                for (int x2 = 0; x2 < w2; ++x2)
                {
                    int x1 = Math.ClampIndex(x2 - 1, width);
                    int i2 = x2 + y2 * w2;
        ...
        {
            if (!IFPipelineSettings.minimalisticLook)
                GUILayout.Space(18f);
        }
